...
RULES:
1. Only include spending (outflows/debits). Skip deposits, transfers between own accounts, and refunds UNLESS they clearly look like income (salary, wages, freelance payments, etc.).
2. The "amount" field must ALWAYS be a positive number representing money spent.
3. Set "confidence" between 0.0 and 1.0 for how sure you are about the category assignment. Use confidence < 0.7 when unsure.
4. If a transaction does not fit any of the available categories, use "bcat-other" as the budgetCategoryId.
5. The "date" field must be in YYYY-MM-DD format. If the year is ambiguous, assume the current year.
6. The "description" field should be a cleaned-up, human-readable version of the original merchant/payee name.
7. The "originalLine" field should contain the original text from the statement for that transaction.
8. "detectedIncome" should be the total of all income/deposit entries you found (salary, wages, interest, etc.), or null if no income was detected.
9. Parse common bank CSV formats — look for columns like date, description/payee/merchant, amount/debit/credit. Also handle plain-text statement formats.
10. Skip header rows, footer rows, balance summaries, and any non-transaction lines.
